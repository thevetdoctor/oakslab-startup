  ...
  async doTask(@Args('task') task: DoTaskDTO): Promise<CompanyTask | any> {
    ...
    if (getPhase.order > 1) {
      ...
      const prevPhases = phases
        ...
        .filter((y) => y.order < getPhase.order);
      ...
      for (const phase of prevPhases) {
        ...
        const prevTasks = tasks
          ...
          .filter((y) => y.order < taskExist.order);

        // Loop through each previous tasks (if any, and if pending)
        if (prevTasks.length) {
          for (const _task of prevTasks) {
            const companyPreviousTaskExist =
              await this.taskService.findCompanyTask({
                task,
                phaseId: phase.id,
              });
            ...
            if (
              !companyPreviousTaskExist ||
              (companyPreviousTaskExist &&
                companyPreviousTaskExist.status === 'pending')
            ) {
              throw new NotFoundException(
                `Task with title "${_task.title}" pending in Phase with title "${phase.title}"`,
              );
            }
          }
        }
      ...
      const tasks = await this.taskService.findAllTasksByPhaseId(
        taskExist.phaseId,
      );
      const prevTasks = tasks
        .map((x) => ({
      ...
      if (prevTasks.length) {
        for (const _task of prevTasks) {
          ...
          if (
            ...
            throw new NotFoundException(
              `Task with title "${_task.title}" pending in Phase with title "${getPhase.title}"`,
            );
          }
        }
      }
    ...
    const companyTaskExist = await this.taskService.findCompanyTask({
      task,
      phaseId: taskExist.phaseId,
    });
    if (!companyTaskExist) {
      return await this.taskService.doTask({
        task,
        phaseId: taskExist.phaseId,
        status: 'done',
      });
